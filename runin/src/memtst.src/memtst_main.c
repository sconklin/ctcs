...
int do_other_processes(int processes, int argc, char **argv, char **env) {
	...
	/* Clear out errno in case it has strange values in it */errno = 0;
...
void usage(char *argv0, char *errormsg) {
	...
	fprintf(
			stderr,
			"usage: %s -12345 -B -c [ceiling] -f [free_vm] -n [processes] -v\n",
			argv0);
	...
	fprintf(stderr,
			"You must specify at least one pattern with -1, -2, et. al\n");
	exit(2);
}
...
int main(int argc, char **argv, char **env) {
	...
	while (1) {
		...
		case 'n':
			if ((processes = strtol(optarg, NULL, 0)) <= 0)
				usage(argv[0],
						"Please specify a number of processes with -n.\n");
			break;
		case '?':
		default:
			usage(argv[0], "");
	...
	memtests = test_setup(patterns, testfunctions_write, testfunctions_check,
			descriptions);
